...
MyGame = ig.Game.extend({
    ...
    setGameSpeed: function() {
        ...
        if (Math.round(this.timePlayed) % 3 == 0) {
            ...
            this.player.maxVel.x += 5;
            this.player.friction.x += 5;
        }
    },
	...
	update: function() {
        ...
        if (!this.startOfLevel) {
            if (this.player.health <= 0) {
				...
				this.player.currentAnim = this.player.anims.death
				if (this.player.currentAnim.loopCount){
                	this.player.kill();
				}
				this.player.currentAnim.update();
				return;
            } else {
				this.time += 1;
				this.itemTimer +=1;
