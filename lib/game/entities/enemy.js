...
.defines(function(){
	...
	EntityEnemy = ig.Entity.extend({
		...
		animSheetExplode: new ig.AnimationSheet('media/ExplosionSpriteSheet.png', 64, 64),
		...
		poo: function(){
			...
			if (drop == 'shield')
				ig.game.spawnEntity(EntityShield, this.pos.x, this.pos.y + offset);
			else if (drop == 'obstacle')
				ig.game.spawnEntity(EntityObstacle, this.pos.x, this.pos.y + offset);
		}
	}),

	EntityEnemyDeath = ig.Entity.extend({
		animSheet: new ig.AnimationSheet('media/ExplosionSpriteSheet.png', 64, 64),
		size: {x: 32, y: 32},
